function robust_geo_verification(strFile1,strFile2, matchingTH)
close all;
...
[H, inliers]=ransacfithomography(x1, x2, thInliers(i));
disp(H/H(3, 3))
disp(['With thershold '  num2str(thInliers(i))]);
disp(['Number of inliers: '  num2str(length(inliers))]);

% REGISTERING AND BLENDING IMAGES
coord1 = x1(:,inliers);
imageRegistration(im1,im2,H,coord1)
end
